...
public class AddProductGUIController {
	...
	public void addItemBtnAction(ActionEvent event) throws ClassNotFoundException, SQLException{
		...
		if(productID.getText().isEmpty() || productName.getText().isEmpty() || categoryMenu.getSelectionModel().getSelectedItem() == null || productQty.getText().isEmpty()) {
			...
			alert.showAndWait();
		}else if (!isInteger(productID.getText())) {
			Alert alert = new Alert(AlertType.ERROR);
			alert.setTitle("Invalid Input");
			alert.setHeaderText(null);
			alert.setContentText("Product ID must be an integer.");
			alert.showAndWait();
		}else if (!isInteger(productQty.getText())) {
			Alert alert = new Alert(AlertType.ERROR);
			alert.setTitle("Invalid Input");
			alert.setHeaderText(null);
			alert.setContentText("Product Qty must be an integer.");
			alert.showAndWait();
		}
		else {
			try {
            ...
            if (idExists) {
                ...
                alert.setHeaderText(null);
                alert.setContentText("The ID you entered is already in use. Please use a unique ID.");
                alert.showAndWait();
                return;
            }
